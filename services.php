        ...
        <?php foreach ($servicesData as $idx => $svc): ?>
          <div class="glass-card <?php echo !empty($svc['highlight']) ? 'highlight-card' : ''; ?>" style="padding: 42px; border-radius: var(--radius-xl);" id="<?php echo htmlspecialchars($svc['id']); ?>">
            <div style="display: grid; grid-template-columns: 1.2fr 1fr; gap: 40px; align-items: center;">
              ...
              <div style="background: #f8fafc; border: 1px solid var(--border-subtle); border-radius: var(--radius-lg); padding: 32px; display: flex; flex-direction: column; gap: 16px;">
                <div style="display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid var(--border-subtle); padding-bottom: 14px;">
                  ...
                  <span style="font-size: 0.75rem; color: #059669; font-weight: 600;">Production Verified</span>
                </div>
                <?php if ($svc['id'] === 'mobile-app-development'): ?>
                  <div style="border-radius: var(--radius-md); overflow: hidden; height: 160px; position: relative;">
                    ...
                    <div style="position: absolute; inset: 0; background: linear-gradient(180deg, rgba(15,23,42,0.1) 0%, rgba(15,23,42,0.8) 100%);"></div>
